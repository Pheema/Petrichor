...
Triangle::Intersect(const Ray& ray, HitInfo* hitInfo) const
{
    ...
    hitInfo->uv =
        weightE1 * (m_vertices[1]->uv - m_vertices[0]->uv) +
...
void Triangle::SampleSurface(Math::Vector3f p, ISampler2D& sampler2D, PointData* pointData, float* pdfArea) const
{
    ...
    const auto v = m_vertices[0]->pos +
        (1.0f - sqrtu0) * e0 + sqrtu0 * u1 * e1;
