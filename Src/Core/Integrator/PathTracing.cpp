...
// TODO:
Color3f SimplifiedSpectrum(float x)
{
    ...
    return Color3f(
        Petrichor::Math::Clamp(result.x, 0.0f, 1.0f),
...
PathTracing::Render(const Scene& scene, Texture2D* targetTex)
{
    ...
    for (int idxTile = 0; idxTile < tileManager.GetNumTiles(); idxTile++)
    {
        ...
        for (int j = j0; j < j0 + tile.GetHeight(); j++)
        {
            for (int i = i0; i < i0 + tile.GetWidth(); i++)
            {
                Color3f pixelColorSum;
                const int kSamplesPerPixel = 128;
                for (int spp = 0; spp < kSamplesPerPixel; spp++)
                {
                    Color3f color;
                    ...
                    {
                        // IBL
                        pixelColorSum += ray.weight * scene.GetEnvironment().GetColor(ray.dir);
                        continue;
                    }
                    ...
                    for (int bounce = 0; bounce < kMaxBounce; bounce++)
                    {
                        ...
                        if (!scene.GetLights().empty())
                        {
                            float pdfArea = 0.0f;
                            auto p = hitInfo.pos;
                            auto pointOnLight = SampleLight(scene, p, rng.next(), rng2D, &pdfArea);
                            Ray rayToLight(p, (pointOnLight.pos - p).Normalized(), RayTypes::Shadow);

                            HitInfo hitInfoOfLight;
                            if (accel.Intersect(rayToLight, &hitInfoOfLight, kEps) &&
                                hitInfoOfLight.hitObj->GetMaterial(rng.next())->GetMaterialType() == MaterialTypes::Emission)
                            {
                                ASSERT(hitInfoOfLight.distance > 0.0f);

                                float eps = (pointOnLight.pos - hitInfoOfLight.pos).SquaredLength();
                                if (Math::ApproxEq(eps, 0.0f, kEps))
                                {
                                    ...
                                    const auto& matEmission = (hitInfoOfLight.hitObj)->GetMaterial(0.0f);
                                    auto li = matEmission->Radiance(rayToLight, rayToLight, hitInfoOfLight);
                                    auto f = mat->BRDF(ray, rayToLight, hitInfo);
                                    auto cos = std::abs(Math::Dot(rayToLight.dir, hitInfo.normal));

                                    float l2 = (hitInfoOfLight.pos - hitInfo.pos).SquaredLength();

                                    float cosP = std::abs(Dot(-rayToLight.dir, hitInfoOfLight.normal));
                                    if (cosP >= 1.0e-4)
                                    {
...
#else
                                        misWeight = pdfDirLight * pdfDirLight / (pdfDirLight * pdfDirLight + pdfDirBSDF * pdfDirBSDF);
#endif
                                    ...
                                    color += misWeight * ray.weight * (li * f * cos / pdfDirLight);
                        ...
                        if (accel.Intersect(ray, &hitInfo, kEps))
                        {
                            ...
                            if (mat->GetMaterialType() == MaterialTypes::Emission)
                            {
                                ...
                                // TODO: 下の関数が確率密度関数を取得しているだけなのに無駄
                                float pdfAreaLight;
                                PointData pointData;
                                (hitInfo.hitObj)->SampleSurface(
                                    ray.o,
                                    rng2D,
                                    &pointData,
                                    &pdfAreaLight
...
#else
                                misWeight = pdfDirBSDF * pdfDirBSDF / (pdfDirLight * pdfDirLight + pdfDirBSDF * pdfDirBSDF);
#endif
                                ASSERT(std::isfinite(misWeight) && misWeight >= 0.0f);
                                color += misWeight * ray.weight * mat->Radiance(ray, ray, hitInfo);
                                ASSERT(color.MinElem() >= 0.0f);
                                break;
                            }
                        }
                ...
                targetTex->SetPixel(i, j, pixelColorSum / static_cast<float>(kSamplesPerPixel));

            }
        }

        std::stringstream ss;
        ss << "[PT: Rendering] " << (idxTile + 1) << "/" << tileManager.GetNumTiles() << std::endl;
        std::cout << ss.str();
    }
}
...
// #pragma optimize("", off)
PointData
PathTracing::SampleLight(const Scene& scene, const Math::Vector3f& p, float randomVal, ISampler2D& sampler2D, float* pdfArea)
{
