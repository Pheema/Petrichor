...
Petrichor::Initialize()
{
    ...
    auto sphere = new Sphere(
        Math::Vector3f(-1.5f, 0.0f, 1.0f),
        1.0f
    );

    float r = 1000.0f;
    auto floor = new Sphere(
        -Math::Vector3f::UnitZ() * r,
        r
    );
    
    auto sphereLight = new Sphere(
        Math::Vector3f(2.5f, -2.0f, 1.0f),
        0.5f
    );
    ...
    MaterialBase* matBodySmooth = new MatMix(matLambertBody, matGGXBodySmooth, 0.2f);

    MaterialBase* matGGXBodyRough = new GGX(0.05f * Color3f::One(), 0.3f);
    MaterialBase* matBodyRough = new MatMix(matLambertBody, matGGXBodyRough, 0.2f);
    ...
    meshBody->Load("Resource/SpeakerDiv3.obj", matArrayBody, 2, ShadingTypes::Smooth);
    meshCover->Load("Resource/CoverMesh.obj", &matMixCover, 1, ShadingTypes::Smooth);
    meshCode->Load("Resource/Code.obj", &matMixCode, 1, ShadingTypes::Smooth);
    meshLightLU->Load("Resource/PanelLightLU.obj", &matLightLU, 1, ShadingTypes::Flat);
    meshLightR->Load("Resource/PanelLightR.obj", &matLightR, 1, ShadingTypes::Flat);
    ...
    auto camera = new Camera(
            Math::Vector3f(-7.53405f, -8.51452f, 5.22279f),
            Math::Vector3f::UnitY()
        );
...
Petrichor::Render()
{
    ...
    uint32_t height = 540;
    auto targetTex = new Texture2D(height*1.5, height);
...
Petrichor::Finalize()
{
    ...
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(timeEnd - m_timeBegin);
    std::cout << "Elapsed time: " << elapsedTime.count() << " msec" << std::endl;
