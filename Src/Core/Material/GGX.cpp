...
GGX::BRDF(const Ray& rayIn, const Ray& rayOut, const HitInfo& hitInfo) const
{
    ...
    // D
    const float dTerm = alpha2 / (M_PI * k * k);
...
GGX::PDF(const Ray& rayIn, const Ray& rayOut, const HitInfo& hitInfo) const
{
    ...
    // D
    const float dTerm = 1.0f / (M_PI * alpha2 * hDotN2 * hDotN2 * k * k);
...
Ray
GGX::CreateNextRay(const Ray& rayIn, const HitInfo& hitInfo, ISampler2D& rng2D, float* pdfDir) const
{
...
Petrichor::Core::MaterialTypes GGX::GetMaterialType(const MaterialBase** mat0 /*= nullptr*/, const MaterialBase** mat1 /*= nullptr*/, float* mix /*= nullptr*/) const
{
    return MaterialTypes::GGX;
}

float GGX::Lambda(const Math::Vector3f& dir, const Math::Vector3f& halfDir) const
{
    const float alpha2 = m_alpha * m_alpha;
...
Math::Vector3f
GGX::SampleGGXVNDF(const Math::Vector3f& dirView, const HitInfo& hitInfo, ISampler2D& rng2D) const
{
    Math::Vector3f normal;
    ...
    const auto t = (std::abs(normal.z) < 0.9999f) ?
        Cross(normal, Math::Vector3f::UnitZ()) : Math::Vector3f::UnitX();
    ...
    const auto t1 = (v.z < 0.9999f) ?
        Cross(v, Math::Vector3f::UnitZ()).Normalized() : Math::Vector3f::UnitX();
    ...
    Math::Vector3f n =
        p1 * t1 +
        p2 * t2 +
        sqrt(std::max(0.0f, 1.0f - p1 * p1 - p2 * p2)) * v;
