...
)
{
    ...
    if (aiscene->HasMeshes())
    {
        ...
        for (size_t idxMesh = 0; idxMesh < aiscene->mNumMeshes; ++idxMesh)
        {
            ...
            std::cout << "[" << path << "] : " << pMesh->mMaterialIndex << std::endl;
            // ---- 頂点の読み込み ----
            for (size_t idxVert = 0; idxVert < pMesh->mNumVertices; ++idxVert)
            {
                ...
                {
                    v.uv = Vector3f(
                        pMesh->mTextureCoords[0][idxVert].x,
        ...
        for (size_t idxMesh = 0; idxMesh < aiscene->mNumMeshes; ++idxMesh)
        {
            ...
            if (pMesh->HasFaces())
            {
                for (size_t idxFace = 0; idxFace < pMesh->mNumFaces; ++idxFace)
                {
                    ...
                    const auto* v0 = &m_vertices[offset + pMesh->mFaces[idxFace].mIndices[0]];
                    const auto* v1 = &m_vertices[offset + pMesh->mFaces[idxFace].mIndices[1]];
                    const auto* v2 = &m_vertices[offset + pMesh->mFaces[idxFace].mIndices[2]];
                    
                    /*std::cout << "[" <<
                        pMesh->mFaces[idxFace].mIndices[0] << ", " <<
                        pMesh->mFaces[idxFace].mIndices[1] << ", " <<
                        pMesh->mFaces[idxFace].mIndices[2] << ", " << std::endl;*/

                    ASSERT(offset + pMesh->mFaces[idxFace].mIndices[0] < m_vertices.size());
                    ASSERT(offset + pMesh->mFaces[idxFace].mIndices[1] < m_vertices.size());
                    ASSERT(offset + pMesh->mFaces[idxFace].mIndices[2] < m_vertices.size());
